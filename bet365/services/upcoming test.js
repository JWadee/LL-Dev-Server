...
const findFixtures = async(fixtures)=>{
  let fixtureIDs = fixtures.map( fixture => {
    // add extra single quote 
    return String("'"+fixture.our_event_id+"'");
  });

  const whereClause = "JSON_EXTRACT(jsonFixture, '$.our_event_id') IN ("+fixtureIDs+");"
  const rows = await fixture.findAll({
    where: Sequelize.literal(whereClause)
  });

  //Check for duplicate fixtures and check which one is newer
  const 
...
const checkUpcoming = () => cron.schedule('*/1 * * * *', async () => {
  //Step 1 - Find leagues
